...
class WikiEnv():
    ...
    def reset(self,data_item):
        ...
        self.golden_pyql = [line for line in data_item.get('pyql').split('\n') if
                            'PyQL()' not in line and '!=' not in line] if data_item.get('pyql') is not None else []
        self.gt_answer.sort()
    ...
    def chain_of_check(self, action_str):
        ...
        if self.self_correction and not success:
            self.post_check(action, params)
    ...
    def validity_check(self, action, params):
        if action == 'add_fact':
            ...
            for k, v in entity_list_temp.items():
                self.entity_list.update({k.lower().replace(' ', ''): v})
            if params[0].lower().replace(' ', '') in self.entity_list.keys():
                bad_entity_id = params[0]
                entity_id = self.entity_list[params[0].lower().replace(' ', '')]
                self.refine_info = f'Refine add_fact({",".join(params)}) to add_fact({",".join([entity_id, params[1], params[2]])})'
                print_refine(self.refine_info)
                params[0] = entity_id
                self.entity_list = entity_list_temp
                return params
            if params[2].lower().replace(' ', '') in self.entity_list.keys():
                bad_entity_id = params[2]
                entity_id = self.entity_list[params[2].lower().replace(' ', '')]
        ...
        if action == 'get_relation':
            ...
            # get_relation(label) -> get_relation(mid)
            entity_list_temp = copy.deepcopy(self.entity_list)
            for k, v in entity_list_temp.items():
                self.entity_list.update({k.lower().replace(' ', ''): v})
            if params[0].lower().replace(' ', '') in self.entity_list.keys():
    ...
    def rationality_check(self, action, params):
        ...
        if action == 'add_fact':
            if (params[0].startswith('?') and params[2].startswith('?')
                ...
                if len(cand_mid) != 0:
                    self.obs = f'You introduced unexisting variable in add_fact(), which is invalid. Please re-generate add_fact() to constrain the same relation for {cand_mid[0]}.'
                else:
                    self.obs = f'You introduced unexisting variable in add_fact(), which is invalid. You should find another approach to solve the question. Please re-generate only Thought {self.steps + 1} and Action {self.steps + 1}.'
                raise ValueError(self.obs)


    def post_check(self, action, params):
        ...
        if action == 'add_fact':
            if self.relation_map == {}:
                ...
                cand_list = sorted(list(set(cand_list)))
                cand_str = ', '.join([f'get_relation({cand})' for cand in cand_list])
                self.obs = f'You should use get_relation() first. Suggestion: choose valid options from {cand_str}. Please check again and re-generate only Thought {self.steps + 1} and Action {self.steps + 1}.'
                raise ValueError(self.obs)
    ...
    def get_relation(self, entity):
        ...
        if success:
            self.obs = {'forward': head_relation, 'backward': tail_relation}
