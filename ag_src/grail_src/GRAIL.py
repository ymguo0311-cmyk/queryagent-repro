...
def grailqa_main():
    ...
    random.Random(233).shuffle(json_list)# read and shuffle data
    ...
    for data_item in json_list[:TEST_LIMIT]:
        ...
        old_time = time.time()
        solve_history = webthink(data_item)# call ReAct loop

        time_list.append(time.time() - old_time)# compute time
